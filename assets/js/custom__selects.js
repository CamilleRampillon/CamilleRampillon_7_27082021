...
import { sort } from "./search__function";
        ...
        for (let k = 0; k < selectOrigine.length; k++) {
          if (selectOrigine.options[k].innerHTML == this.innerHTML) {
            ...
            if (selectValue === "ingredient") {
              sortValueTab.splice(0, 1, this.innerHTML);
              sort(sortValueTab[0]);
            } if (selectValue === "appliance") {
              sortValueTab.splice(1, 1, this.innerHTML);
              sort(sortValueTab[1]);
            } if (selectValue === "ustensil") {
              sortValueTab.splice(2, 1, this.innerHTML);
              sort(sortValueTab[2]);
            }
            sortValue = this.innerHTML;
...
function tagsShower(value){
  ...
  tag.innerHTML = `<span>${sortValue}</span><i class ="far fa-times-circle closeBtn" id="btn${value}"></i>`;
  ...
  closeBtn.forEach((btn) => btn.addEventListener('click', (e) => {
    ...
    } else if (sortValueTab[1] != "") {
      sort(sortValueTab[1]);
      if (sortValueTab[2] != "") {
        sort(sortValueTab[2]);
      }
    } else if (sortValueTab[2] != "") {
        sort(sortValueTab[2]);
    }
  }))
}