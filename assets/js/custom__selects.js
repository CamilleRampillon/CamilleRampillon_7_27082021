...
export let customisation = function () {
  ...
  for (let i = 0; i < customSelect.length; i++) {
    ...
    } else if (customSelect[i].getAttribute("class") === "custom-select ustensils") {
      ...
      function trier() {
        ...
        for (let k = 0; k < selectOrigine.length; k++) {
          if (selectOrigine.options[k].innerHTML == this.innerHTML) {
            ...
            sortValueTab.push(this.innerHTML)
            sortValueTab.forEach((value) => tagSort(value))
            sortValueTab=sortValueTab.filter((elmt)=>elmt!="")
            sortValue = this.innerHTML;
            tagsShowerFromTag(selectValue, sortValue);
            const sameSelected =
